 ...
 * Production-ready: handles failures gracefully, validates responses
 */

import { logError, logInfo, logWarn } from './logger';
import { withRetry, withTimeout, CircuitBreaker } from './utils';
import { 
  safeValidate, 
...
Return ONLY valid JSON array, no markdown fences.`;
  ...
  try {
    const response = await withRetry(
      () => callGemini(config, prompt, systemPrompt),
      {

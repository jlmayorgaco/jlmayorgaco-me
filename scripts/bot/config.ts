 ...
 * Production-ready: uses process.env, validates with Zod, no secrets in files
 */
...
const BotConfigSchema = z.object({
  ...
  telegramRateLimit: z.number().default(30), // messages per minute
...
export async function loadConfig(): Promise<BotConfig> {
  ...
  // Otherwise, try to load from config file (for backwards compatibility)
  const fileConfig = await loadFromFile();
  
  // Merge: Environment > File > Defaults
  const mergedConfig = {
    telegram: {
...
async function loadFromFile(): Promise<Partial<BotConfig>> {
  ...
  try {
    ...
    // Log warning about deprecated config file
    console.warn('⚠️  WARNING: Using .env.json is deprecated. Please migrate to environment variables.');
    console.warn('   Set TELEGRAM_BOT_TOKEN, GEMINI_API_KEY, and TELEGRAM_CHAT_ID as environment variables.');
 ...
 * Use this in CI/CD pipelines or pre-deployment checks
 */
...
/**
 * Get config without sensitive data (for logging/debugging)
 */
